...
function ent.PlayerFilter( pl )
	...
	ent.plys[pl].IsFriend 			= false; //pl:GetFriendStatus( ) == "friend";
...
CreateClientConVar( "xhit_aim_antiantiaim", 1, false, false );
...
function aim:Aimbot( ucmd )
	   ...
	   if( ucmd:KeyDown( IN_ATTACK ) ) then
		...
		if( ply:GetMoveType() != MOVETYPE_LADDER && ucmd:KeyDown( IN_JUMP ) && ply:OnGround() == false ) then
		...
		local targ = aim.info.target;
		if( GetConVarNumber( "xhit_aim_antiantiaim" ) == 1 ) then
			if( _R.Entity.EyeAngles ) then
				local ang = targ:EyeAngles();
				if ( ( ang.p > 180.087936 )
					&& ( ang.p <= 271.000000 ) ) then
					targ:ManipulateBoneAngles( targ:LookupBone( "ValveBiped.Bip01_Head1" ), Angle( 271, ang.y, ang.r ) );
				elseif( ( ang.p >= 89.000000 )
					&& ( ang.p <= 180.087936 ) ) then
					targ:ManipulateBoneAngles( targ:LookupBone( "ValveBiped.Bip01_Head1" ), Angle( 89, ang.y, ang.r ) );
				end
			end
		end
		
		local angles = targ:GetBonePosition( targ:LookupBone( "ValveBiped.Bip01_Head1" ) );//ent:GetHeadPos2( aim.info.target ); //1
...
hook.Add('CalcView','zzzzzzzzznovisrecoil', novisrecoil)
