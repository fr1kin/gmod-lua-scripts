...
package.loaded.scriptenforcer2 = nil
...
function PB:AddNormalHook( typ, name, func )
	...
	return hook.Add( typ, name, func ), PB:ChatMessage( "[ADDED] Hook: " .. name .. " ["..typ.."]" )
end

function PB:AddHook( typ, func, notes )
	...
	return hook.Add( typ, ran, func ), PB:ConsoleMessage( "Added Hook: " .. ran .. " ["..typ.."]" .. " {" .. msg .. "}" )
end
...
PB:AddHook( "Think", PB.Released, "Released check." )
...
// Force a CVar (gm_funcsolver).

require( "fuckoff2" )
package.loaded.fuckoff2 = nil

function ForceCVar( name, val )
	if ( ConVarExists( name ) ) then
		local GCVN = GetConVarNumber( name )
		table.insert( PB.CVars, { cvar = name, num = GCVN } )
		return SetConvar( CreateConVar( name, "" ), val ), PB:ConsoleMessage( "CVar ('" .. name .. "') has been set to: " .. val )
	end
	return PB:ConsoleMessage( "CVar ('" .. name .. "') does not exist!" )
end
...
PB:AddHook( "Think", PB.HasWeapon, "TTT Class check." )

function PB:DetectClasses( e )
...
function PB:SetColors( e )
	...
	if ( e:IsPlayer() ) then
		local ttt_col = PB:DetectClasses( e )
		col = ttt_col
...
function PB:GenerateSpot( e )
	...
	local spt = e:LocalToWorld( e:OBBCenter() ), e:LocalToWorld( e:OBBCenter() )
	
	// Main Player Bones:
	if ( AimSpot:GetInt() == 1 ) then spt = e:GetBonePosition( e:LookupBone( Bones.Head ) ) end
	if ( AimSpot:GetInt() == 2 ) then spt = e:GetBonePosition( e:LookupBone( Bones.Neck ) ) end
	if ( AimSpot:GetInt() == 3 ) then spt = e:GetBonePosition( e:LookupBone( Bones.Spine ) ) end
	if ( AimSpot:GetInt() == 4 ) then spt = e:GetBonePosition( e:LookupBone( Bones.Spine1 ) ) end
	if ( AimSpot:GetInt() == 5 ) then spt = e:GetBonePosition( e:LookupBone( Bones.Spine2 ) ) end
	if ( AimSpot:GetInt() == 6 ) then spt = e:GetBonePosition( e:LookupBone( Bones.Ass ) ) end
	if ( AimSpot:GetInt() == 7 ) then spt = e:GetBonePosition( e:LookupBone( Bones.RForearm ) ) end
	if ( AimSpot:GetInt() == 8 ) then spt = e:GetBonePosition( e:LookupBone( Bones.RHand ) ) end
	if ( AimSpot:GetInt() == 9 ) then spt = e:GetBonePosition( e:LookupBone( Bones.LForearm ) ) end
	if ( AimSpot:GetInt() == 10 ) then spt = e:GetBonePosition( e:LookupBone( Bones.LHand ) ) end
	if ( AimSpot:GetInt() == 11 ) then spt = e:GetBonePosition( e:LookupBone( Bones.RCalf ) ) end
	if ( AimSpot:GetInt() == 12 ) then spt = e:GetBonePosition( e:LookupBone( Bones.RFoot ) ) end
	if ( AimSpot:GetInt() == 13 ) then spt = e:GetBonePosition( e:LookupBone( Bones.LCalf ) ) end
	if ( AimSpot:GetInt() == 14 ) then spt = e:GetBonePosition( e:LookupBone( Bones.LFoot ) ) end
	
	// Model Targets:
	local m = e:GetModel()
	if ( m == "models/crow.mdl" || m == "models/pigeon.mdl" ) then spt = e:LocalToWorld( Vector( 0, 0, 5 ) ) end
	if ( m == "models/seagull.mdl" ) then spt = e:LocalToWorld( Vector( 0, 0, 6 ) ) end
	if ( m == "models/combine_scanner.mdl" ) then spt = e:GetBonePosition( e:LookupBone( "Scanner.Body" ) ) end
	if ( m == "models/hunter.mdl" ) then spt = e:GetBonePosition( e:LookupBone( "MiniStrider.body_joint" ) ) end
	if ( m == "models/combine_turrets/floor_turret.mdl" ) then spt = e:GetBonePosition( e:LookupBone( "Barrel" ) ) end
	if ( m == "models/dog.mdl" ) then spt = e:GetBonePosition( e:LookupBone( "Dog_Model.Eye" ) ) end
	if ( m == "models/vortigaunt.mdl" ) then spt = e:GetBonePosition( e:LookupBone( "ValveBiped.Head" ) ) end
	if ( m == "models/antlion.mdl" ) then spt = e:GetBonePosition( e:LookupBone( "Antlion.Body_Bone" ) ) end
	if ( m == "models/antlion_guard.mdl" ) then spt = e:GetBonePosition( e:LookupBone( "Antlion_Guard.Body" ) ) end
	if ( m == "models/antlion_worker.mdl" ) then spt = e:GetBonePosition( e:LookupBone( "Antlion.Head_Bone" ) ) end
	if ( m == "models/zombie/fast_torso.mdl" ) then spt = e:GetBonePosition( e:LookupBone( "ValveBiped.HC_BodyCube" ) ) end
	if ( m == "models/zombie/fast.mdl" ) then spt = e:GetBonePosition( e:LookupBone( "ValveBiped.HC_BodyCube" ) ) end
	if ( m == "models/headcrabclassic.mdl" ) then spt = e:GetBonePosition( e:LookupBone( "HeadcrabClassic.SpineControl" ) ) end
	if ( m == "models/headcrabblack.mdl" ) then spt = e:GetBonePosition( e:LookupBone( "HCBlack.body" ) ) end
	if ( m == "models/headcrab.mdl" ) then spt = e:GetBonePosition( e:LookupBone( "HCFast.body" ) ) end
	if ( m == "models/zombie/poison.mdl" ) then spt = e:GetBonePosition( e:LookupBone( "ValveBiped.Headcrab_Cube1" ) ) end
	if ( m == "models/zombie/classic.mdl" ) then spt = e:GetBonePosition( e:LookupBone( "ValveBiped.HC_Body_Bone" ) ) end
	if ( m == "models/zombie/classic_torso.mdl" ) then spt = e:GetBonePosition( e:LookupBone( "ValveBiped.HC_Body_Bone" ) ) end
	if ( m == "models/zombie/zombie_soldier.mdl" ) then spt = e:GetBonePosition( e:LookupBone( "ValveBiped.HC_Body_Bone" ) ) end
	if ( m == "models/combine_strider.mdl" ) then spt = e:GetBonePosition( e:LookupBone( "Combine_Strider.Body_Bone" ) ) end
	if ( m == "models/combine_dropship.mdl" ) then spt = e:GetBonePosition( e:LookupBone( "D_ship.Spine1" ) ) end
	if ( m == "models/combine_helicopter.mdl" ) then spt = e:GetBonePosition( e:LookupBone( "Chopper.Body" ) ) end
	if ( m == "models/gunship.mdl" ) then spt = e:GetBonePosition( e:LookupBone( "Gunship.Body" ) ) end
	if ( m == "models/lamarr.mdl" ) then spt = e:GetBonePosition( e:LookupBone( "HeadcrabClassic.SpineControl" ) ) end
	if ( m == "models/mortarsynth.mdl" ) then spt = e:GetBonePosition( e:LookupBone( "Root Bone" ) ) end
	if ( m == "models/synth.mdl" ) then spt = e:GetBonePosition( e:LookupBone( "Bip02 Spine1" ) ) end
	if ( m == "mmodels/vortigaunt_slave.mdl" ) then spt = e:GetBonePosition( e:LookupBone( "ValveBiped.Head" ) ) end
...
Cones 		  = {}
Cones.Weapons = {}
Cones.Weapons[ "weapon_pistol" ]	= WeaponVector( 0.0100, true )	// HL2 Pistol
Cones.Weapons[ "weapon_smg1" ]		= WeaponVector( 0.04362, true )	// HL2 SMG1
Cones.Weapons[ "weapon_ar2" ]		= WeaponVector( 0.02618, true )	// HL2 AR2
Cones.Weapons[ "weapon_shotgun" ]	= WeaponVector( 0.08716, true )	// HL2 SHOTGUN
...
function GetCone( wep )
	...
	if ( !c && ( type( wep.Primary ) == "table" ) && ( type( wep.Primary.Cone ) == "number" ) ) then c = wep.Primary && wep.Primary.Cone end
	if ( !c ) then c = 0 end
...
function PB.Nospread( comd, angle )
	...
	if ( w && w:IsValid() && ( type( w.Initialize ) == "function" ) ) then
		...
		if ( type( valCone ) == "number" ) then
			vecCone = WeaponVector( valCone, true )
...
local function ValidTarget( e )
	local ply = LocalPlayer()
		if ( !ValidEntity(e) ) || ( e == ply ) || ( AimPlayer:GetBool() && !e:IsPlayer() ) || ( !e:IsNPC() && !e:IsPlayer() ) then return false end
		
		local m = e:GetMoveType()	
		if ( e:GetMoveType() == MOVETYPE_NONE ) then return false end
		if ( e:GetMoveType() == 10 ) then return false end
		if ( e:IsPlayer() ) then
			if ( !e:Alive() ) || ( m == MOVETYPE_OBSERVER ) || ( e:Team() == ply:Team() && AimFriendlyFire:GetBool() ) then return false end
			if ( AimSteam:GetBool() ) && e:GetFriendStatus() == "friend" then return false end
			
			//local typ = e:SteamID()
			//if ( e:SteamID() == NULL || e:SteamID() == "BOT" || e:IsBot() ) then typ = e:Nick() else typ = e:SteamID() end
			if ( table.HasValue( PB.Friends, e:Nick() ) ) then return false end
...
function PB.Bot( u )
		...
		*/
		...
		setAng = setAng + ( tar:GetVelocity() / offset - ply:GetVelocity() / offset )
		
		local ang = ( setAng-myPos ):Angle()
		
		if ( RemSpread:GetBool() ) then
			ang = PB.Nospread( u, ang )
		end
	...
	function PB.AimHUD()
		...
		for k, e in pairs( ents.GetAll() ) do
			if ( tar != nil && e == tar ) then
				local spot = PB:GenerateSpot( e ) + ( tar:GetVelocity() / offset - ply:GetVelocity() / offset )
	...
	PB:AddHook( "CreateMove", PB.Bot, "Aimbot function" )
	PB:AddHook( "HUDPaint", PB.AimHUD, "Aimbot HUD" )
	PB:AddHook( "CalcView", PB.NoRecoil, "Norecoil Script" )
/*--------------------------------------------------
...
PB:AddHook( "HUDPaint", PB.Extrasensory, "ESP Script" )
...
PB:AddHook( "RenderScreenspaceEffects", PB.Walls, "Wallhack script." )
...
PB:AddHook( "HUDPaint", PB.Visuals, "Visuals scripts." )

// Small function to find and print all the admins.
function PrintAdmin()
	for k, v in pairs( player.GetAll() ) do
		if ( PB:IsAdmin(v) ) then 
			print( v:Nick() .. "\n" )
		end
	end
end
PB:AddConCommand( "pb_find_admins", PrintAdmin )

// Speedhack
require( "fuckoff2" )
PB:AddConCommand( "+pb_speed", function()
	ForceCVar( "host_framerate", GetConVarString( "pb_misc_speedvalue" ) )
end )
PB:AddConCommand( "-pb_speed", function()
	ForceCVar( "host_framerate", 0 )
end )

PB:AddConCommand("+pb_hop", function()
	hook.Add("Tick", "Bhasda", function()
...
function PB.FunctionAdminlist()
	...
	return admins, adminnum, wide, tall, posX, posY
end
PB:AddHook( "Think", PB.FunctionAdminlist, "Admin Check" )
...
function OpenVGUI()
	...
	if ( n == 1 ) then
		OtherVGUI()
		n = n - 999999999999999999 // I hope I don't open the menu 999999999999999999 times!
	end
end
	...
	local mdButton2 = vgui.Create( "DButton" )
		...
		mdButton2.DoClick = function()
			...
			if ( D ) then
				...
				ForceCVar( "sv_allow_voice_from_file", 1 )
				PB:ChatMessage( "Enabled HLDJ/HLSS" )
			else
				ForceCVar( "sv_allow_voice_from_file", 0 )
...
function engineConsoleCommand( ply, cmd, args )
	...
	for k, v in pairs( PB.ConVars ) do
		if string.find( string.lower( v ), cmd ) then
			return PB:ConsoleMessage( "Spoofed - " .. cmd .. " | engineConsoleCommand" )
		end
	end
	
	if string.find( string.lower( "sv_cheats" ), cmd ) then return 0 end
	if string.find( string.lower( "host_framerate" ), cmd ) then return 1 end
	if string.find( string.lower( "sv_allow_voice_from_file" ), cmd ) then return 0 end
	
	return old_eccommand( ply, cmd, args )
end

local ent = _R.Entity

local getMat = ent.GetMaterial
local materials = { "effects/ar2_altfire1" }

function ent:GetMaterial()
	local maT = getMat( self )
		for k, v in pairs( materials ) do
			if string.find( string.lower( maT ), v ) then
				return;
			end
		end
	return maT or ""
...
local p = _R.Player
local myFrags = p.Frags()

function p:Frags()
	local old_frags = myFrags( self )
	if ( ServerGamemodeIs( "darkrp" ) ) then
		return 0
	end
	return old_frags || 0
end
