...
local function PlayerDetected( r )
        net.Start( NET_MSG );
                net.WriteInt( r || -1, 32 );
        net.SendToServer();
...
local function Scan()
	if( ConVarExists( "sv_allowcslua" ) == false ) then
		PlayerDetected( AC_NOEXIST );
        return;
        end
        if( GetConVarNumber( "sv_allowcslua" ) != 0 ) then
                PlayerDetected( AC_FORCEDCVAR );
                return;
        end
       ...
       if( SizeOf( modules ) == 0 ) then
              modules, folders = file.Find( "lua/bin/*", "GAME" );
       end
        if( modules ) then
