...
function _R.Entity:FireBullets( bullet )
	...
	BULLET[wep:GetClass()] = bullet.Spread;
	return oldBullets( self, bullet );
end
...
do
	local pos, flpitch;
	 function ent:GetHeadPos2( entity )
		...
		//end
		/*flpitch = ent:EyeAngles().p  || Angle(0)
...
function aim:Aimbot( ucmd )
		...
		if aim.info.target:GetColor().a == 200  then return false
				//continue;
			end
...
local function novisrecoil(ply,origin,angles,fov)
	...
	local view = {} 
	view.origin = ply:GetShootPos() - LocalPlayer():GetAimVector():Angle():Forward() 
