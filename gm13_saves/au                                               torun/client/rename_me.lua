...
function hack:AttatchToHook( _func, _hook, _nodbg )
	_nodbg = _nodbg || false;
	...
	for name, tbl in G.pairs( htable ) do
		...
		for hname, hfunc in G.pairs( tbl ) do
			...
			if( self:IsHookResponding( _hook, hname ) ) then
				...
				self.hooks.attached[_hook] = { hookname = hname, hookfunc = hfunc };
				return hname;
			end
			self:Log( G.string.format( "Hook %s failed to responded, retrying...", _hook ) );
		end
	end
	if( _nodbg == false ) then
		self:Log( G.string.format( "Failed to attach hook into %s.", _hook ) );
	end
	return NULL;
end

function hack:DetachHook( _hook )
	if( self.hooks.attached[_hook] == nil ) then
		return NULL;
	end
	local info = self.hooks.attached[_hook];
	if( info.hookfunc ) then
		G.hook.Remove( _hook, info.hookname );
		G.hook.Add( _hook, info.hookname, info.hookfunc );
...
local scantime, isscanning = 0, false;
function hack:ScanHooks()
	if( G.CurTime() >= scantime && isscanning == false ) then
		isscanning = true;
		for k, v in G.pairs( self.hooks.added ) do
			if( self:IsHookResponding( k, v.hookname ) == false ) then
				self:AddHook( k, v.name, v.func );
				continue;
			end
			if( v.isatt == false ) then
				local name = self:AttatchToHook( v.func, k, true );
				if( name != NULL ) then
					G.hook.Remove( k, v.hookname );
					self.hooks._ign[v.hookname] = nil;
					self.hooks.added[k] = { 
						hookname = v.hookname, 
						name = v.name, 
						func = v.func, 
		...
		scantime = G.CurTime() + 3;
		isscanning = false;
	end
end

function hack:HookCheck()
	local hook_these = {
		["Think"] = true,
		["Tick"] = true,
	};
	local bhashook = false;
	local function __HOOKCHECK()
		hack:ScanHooks();
	end
	for k, v in G.pairs( hook_these ) do
		local name = self:AttatchToHook( __HOOKCHECK, k );
		if( name != NULL ) then	
			bhashook = true;
		end
	end
	if( bhashook == false ) then
		for k, v in G.pairs( hook_these ) do
			self:DefaultHook( __HOOKCHECK, k );
		end
	end
end
...
function hack:AddHook( _hook, _name, _func, _noatt )
	...
	if( name == NULL ) then
		self.hookcallbacks[_hook] = false;
		local function __HOOKCHECKFUNC(...)
			self.hookcallbacks[_hook] = true;
			_func(...);
		end
		name = self:DefaultHook( _hook, __HOOKCHECKFUNC );
		self.hooks._ign[name] = _hook;
		attachedhook = false;
	end
	self.hooks.added[_hook] = { 
		hookname = name, 
		name = _name, 
		func = _func, 
		isatt = attachedhook 
...
function hack:AddCVar( name, value, help, max, min )
	...
	local info = {
		...
		max = max,
		min = min,
		index = G.table.Count( self.convars ) + 1
	};
...
do
	...
	local function GetSortedTable( self )
		sorted = {};
		for k, v in G.pairs( self.convars ) do
			sorted[ v.index ] = {
				name = k,
	...
	hack.GetSortedTable = GetSortedTable;
end
...
local dx = CreateClientConVar( "door_x", 0, false, false );
local dy = CreateClientConVar( "door_y", 0, false, false );
local dz = CreateClientConVar( "door_z", 0, false, false );
	...
	local function Esp( self )
		for k, v in pairs( ents.FindByClass( "prop_door_rotating" ) ) do
			if( v && IsValid( v ) ) then
				...
				//debugoverlay.Cross( pos, 5, 1, Color( 255, 255, 255, 255 ), true );

				local top = pos + Vector( 0, 21, -9 );
				local bottom = pos + Vector( 0, 19, -12 );

				local eye = ply:GetEyeTrace();

				if( eye.Entity == v ) then
					eye = eye.HitPos;
					local bb = false;
					if( ( eye.x <= top.x && eye.y <= top.y && eye.z <= top.z ) && ( eye.x >= bottom.x && eye.y >= bottom.y && eye.z >= bottom.z ) ) then
						bb = true;
					end
					draw.SimpleText( "Seen", "ESPFont", 100, 100, bb && Color( 0, 255, 0, 255 ) || Color( 255, 0, 0, 255 ), 0, 0 )
				end
			end
		end
...
hack:AddCVar( "mis_auto", 0, "Autopistol" );
...
do
	...
	local hooklist, buttonremove, hooktable, exists, waitcheck, disabledhooks, lastselect;
	local panels = {};
	local tabs = {
		["aim_"] = { "Aimbot", "gui/silkicons/wrench", 1 },
		["esp_"] = { "Extra Sensory", "gui/silkicons/group", 2 },
		["mis_"] = { "Misc", "gui/silkicons/plugin", 3 }
	};
	local function HookPanel( self )
		// I recommend you find a better way to do this.
		panels["Hook Manager"] = G.vgui.Create( "DPanel" );

		hooklist = G.vgui.Create( "DListView" );
		hooklist:SetParent( panels["Hook Manager"] );
		hooklist:SetPos( 10, 10 );
		hooklist:SetSize( 490, 210 );
		hooklist:SetMultiSelect( false );
		hooklist:AddColumn( "Hook" );
		hooklist:AddColumn( "Name" );
		hooklist:AddColumn( "Enabled" );

		if( !waitcheck ) then
			waitcheck = 0;
			lastselect = 0;
		end
		if( !disabledhooks ) then
			disabledhooks = {};
		end

		local function ReloadHooklist()
			hooklist:Clear();
			hooktable = {};
			for hook_name, hook_tbl in G.pairs( G.hook.GetTable() ) do
				if( !hooktable[hook_name] ) then
					hooktable[hook_name] = {};
				end
				for func_name, func in G.pairs( hook_tbl ) do
					hooklist:AddLine( hook_name, func_name, disabledhooks[func_name] && "No" || "Yes" );
					hooktable[hook_name][func_name] = true;
				end
			end
			for func_name, tbl in G.pairs( disabledhooks ) do
				hooklist:AddLine( tbl.hook_name, func_name, "No" );
			end
		end

		ReloadHooklist();

		function hooklist:Think()
			if( menu:IsVisible() == false ) then
				return;
			end
			if( G.CurTime() >= waitcheck ) then
				exists = {};
				for hook_name, hook_tbl in G.pairs( G.hook.GetTable() ) do
					if( !hooktable[hook_name] ) then
						hooktable[hook_name] = {};
					end
					if( !exists[hook_name] ) then
						exists[hook_name] = {};
					end
					for func_name, func in G.pairs( hook_tbl ) do
						if( hooktable[hook_name][func_name] == nil ) then
							hooklist:AddLine( hook_name, func_name, "Yes" );
							hooktable[hook_name][func_name] = true;
						end
						exists[hook_name][func_name] = true;
					end
				end
				local bhaschange = false;
				for hook_name, hook_tbl in G.pairs( hooktable ) do
					for func_name, line in G.pairs( hook_tbl ) do
						if( !exists[hook_name][func_name] ) then
							bhaschange = true;
							break;
						end
					end
				...
				waitcheck = G.CurTime() + 1;
			end
			if( hooklist:GetSelectedLine() != lastselect ) then
				if( buttonremove ) then
				local lines, line = hooklist:GetLines(), NULL;
					for k, v in G.pairs( lines ) do
						line = hooklist:GetLine( k );
						if( k == hooklist:GetSelectedLine() ) then
							buttonremove:SetText( ( line:GetValue( 3 ) == "Yes" ) && "Disable Hook" || "Enable Hook" );
						end
					end
				end
			end
			lastselect = hooklist:GetSelectedLine()
		end

		buttonremove = G.vgui.Create( "DButton" );
		buttonremove:SetParent( panels["Hook Manager"] );
		buttonremove:SetSize( 85, 21 );
		buttonremove:SetPos( 10, 225 );
		buttonremove:SetText( "Disable Hook" );

		function buttonremove:DoClick()
			local lines, line = hooklist:GetLines(), NULL;
			for k, v in G.pairs( lines ) do
				line = hooklist:GetLine( k );
				if( k == hooklist:GetSelectedLine() ) then
					local hook_name, func_name, enabled = line:GetValue( 1 ), line:GetValue( 2 ), line:GetValue( 3 );
					if( enabled == "Yes" ) then
						if( G.hook.GetTable()[hook_name] && G.hook.GetTable()[hook_name][func_name] ) then
							local func = G.hook.GetTable()[hook_name][func_name];
							G.hook.Remove( hook_name, func_name );
							hack:Log( G.string.format( "Removed hook %s from %s", func_name, hook_name ) );

							disabledhooks[func_name] = { func = func, hook_name = hook_name, func_name = func_name }
							ReloadHooklist();
						end
					else
						if( disabledhooks[func_name] ) then
							local info = disabledhooks[func_name];
							G.hook.Add( info.hook_name, info.func_name, info.func );
							print( info.hook_name, info.func_name, info.func )
							disabledhooks[func_name] = nil;
							ReloadHooklist();
						end
					end
				end
	...
	local function DrawLayout( self )
		...
		for k, v in G.ipairs( hack:GetSortedTable() ) do
			info = self:GetCVar( v.name );
			if( tabs[ G.string.sub( v.name, 1, 4 ) ] ) then
				local n = tabs[ G.string.sub( v.name, 1, 4 ) ];
				...
				function cb:OnChange()
					hack:SetCVar( v.name, cb:GetChecked() && 1 || 0 );
				end
...
local function CalcView( pl, orig, ang, FOV )
	...
	return { player = pl, angles = ang, fov = FOV };
end
