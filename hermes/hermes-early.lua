...
_G.setmetatable( _G, { __metatable = true } )
...
function Hermes.Menu()
	...
	Hermes.set.panel:SetPos( ScrW() / 2 - 400 / 2, ScrH() / 2 - 300 / 2 )
	Hermes.set.panel:SetSize( 400, 300 )
	...
	menu:SetParent( Hermes.set.panel )
	menu:SetPos( 10, 55 )
	menu:SetSize( 380, 235 )
	menu:EnableVerticalScrollbar()
	
	local aim = vgui.Create( "DPanelList" )
	aim:SetVisible( true )
	aim:SetParent( menu )
	aim:SetPos( 10, 10 )
	aim:SetSize( 355, 245 )
	aim:EnableVerticalScrollbar()
	aim.Paint = function() end
	...
	esp:SetPos( 10, 10 )
	esp:SetSize( 355, 245 )
	esp:EnableVerticalScrollbar()
	esp.Paint = function() end
	
	local abutton = vgui.Create( "DButton" )
	abutton:SetParent( Hermes.set.panel )
	abutton:SetSize( 50, 20 )
	abutton:SetPos( 10, 30 )
	abutton:SetText( "Aimbot" )
	abutton.DoClick = function() aim:SetVisible( true ) esp:SetVisible( false ) end
	
	local ebutton = vgui.Create( "DButton" )
	ebutton:SetParent( Hermes.set.panel )
	ebutton:SetSize( 50, 20 )
	ebutton:SetPos( 65, 30 )
	ebutton:SetText( "ESP" )
	ebutton.DoClick = function() aim:SetVisible( false ) esp:SetVisible( true ) end
	...
	for k, info in ipairs( Hermes.menuinfo ) do
		if ( info.Type == "boolean" ) then
			...
			if ( info.Menu == "aim" ) then
				aim:AddItem( checkbox )
			elseif ( info.Menu == "esp" ) then
				esp:AddItem( checkbox )
			end
