...
HERMES:AddSetting( "misc_spectator", false, "Spectatorlist", "Misc" )
HERMES:AddSetting( "misc_gag", true, "Antigag", "Misc" )
HERMES:AddSetting( "misc_bhop", true, "Bunnyhop", "Misc" )
HERMES:AddSetting( "misc_autopistol", false, "Autopistol", "Misc" )
...
HERMES:AddSetting( "misc_nosky", false, "Nosky", "Removals" )
...
/* --------------------
	:: Spectatorlist
*/ --------------------
function HERMES:ToggleSpectatorlist()
	local spec = HERMES:GetSpeclist()
	if( !HERMES.item['spectatorlist'] && spec:IsVisible() ) then
		spec:SetVisible( false )
	elseif( HERMES.item['spectatorlist'] && !spec:IsVisible() ) then
...
function HERMES:Removals()
	...
	if( HERMES.item['nosky'] ) then
		HERMES.hermes.ForceVar( "gl_clear", 1.0 )
		HERMES.hermes.ForceVar( "_hermes_r_drawskybox", 0.0 )
		HERMES.hermes.ForceVar( "_hermes_r_3dsky", 0.0 )
	else
		HERMES.hermes.ForceVar( "_hermes_r_drawskybox", 1.0 )
		HERMES.hermes.ForceVar( "_hermes_r_3dsky", 1.0 )
	end
end
...
*/ --------------------
local loopn = 0
function HERMES:GetAllEnts()
	if( loopn > 5 ) then
		for k, e in ipairs( ents.GetAll() ) do
			if( ValidEntity( e ) && ( e:IsPlayer() || e:IsNPC() ) ) then
				HERMES.getall[k] = e
			end
		end
		loopn = 0
