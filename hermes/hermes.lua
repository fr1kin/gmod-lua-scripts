...
Hermes:CreateClientConVar( "misc_speedhack", 1, "speedhackspeed", "Speedhack Amount", "misc", 0, 10, false )
		...
		if ( ValidEntity( e ) && e != ply ) then
			...
			for i = 1, table.Count( weps ) do
				...
				if ( ValidEntity( w ) ) then
					if ( table.HasValue( Hermes.traitorsweps, w:GetClass() ) && !table.HasValue( Hermes.traitors, e:Nick() ) ) then
...
function Hermes:CreatePos( e )
	...
	local d, v, t = math.Round( e:GetPos():Distance( ply:GetShootPos() ) )
	...
	if ( Hermes:GetValue( Hermes.features.otherpos, 0 ) ) then
		...
		pos = e:LocalToWorld( e:OBBMaxs() ) + Vector( 0, 0, v + 10 )
...
function Hermes:ValidTargets( e )
	...
	if ( e:IsPlayer() && e:IsVehicle() && Hermes:GetValue( Hermes.features.vehicleaim, 0 ) ) then return false end
...
function Hermes:GetTarget()
	...
	for i = 1, table.Count( ent ) do
		...
		if ( !ValidEntity( e ) || !Hermes:ValidTargets( e ) ) then return 0 end
...
function Hermes.Bunnyhop( ucmd )
	local ply = LocalPlayer()
	if ( Hermes:GetValue( Hermes.features.bunnyhop, 1 ) && ( ucmd:GetButtons() & IN_JUMP ) > 0 ) then
		if ( ply:IsOnGround() ) then
			ucmd:SetButtons( ucmd:GetButtons() | IN_JUMP )
		else
			ucmd:SetButtons( ucmd:GetButtons() & ( 0xFFFF - IN_JUMP ) )
		end
	end
end
...
function Hermes.Autopistol( ucmd )
	local ply = LocalPlayer()
	if ( Hermes:GetValue( Hermes.features.autopistol, 1 ) && input.IsMouseDown( MOUSE_LEFT ) ) then
		if ( shoot ) then
			Hermes.old.rcc( "+attack" )
			shoot = false
		elseif ( !shoot ) then
	...
	elseif ( Hermes:GetValue( Hermes.features.autopistol, 1 ) && !input.IsMouseDown( MOUSE_LEFT ) ) then
		if ( !shoot ) then
			Hermes.old.rcc( "-attack" )
			shoot = true
		end
...
function Hermes.Aim( ucmd )
	...
	if ( Hermes:GetValue( Hermes.features.afterkill, 1 ) && Hermes.set.killed ) then Hermes.set.aiming = false Hermes.set.killed = false end
	
	timer.Simple( 0.01, function()
		if ( !Hermes.DeadTarget( target ) ) then
			MsgN( "Dead Target!" )
			Hermes.set.killed = true
		end
	end )
	
	local aim, myPos, setView = Hermes:TargetLocation( target ), ply:GetShootPos(), 0
	aim = aim + ( target:GetVelocity() / 45 - ply:GetVelocity() / 45 ) + ( Vector( 0, 0, 0.0000000000000000001 ) )
