...
if ( Hermes ) then _G.Hermes = nil end -- I know doing a local again will override a global, but I'm just a fag okay.
...
Hermes.getcvars = {}
Hermes.convars = { -- INCOMING SHIT STORM
	{ Item = "Aimbot", Setting = {
			{ Tab = "Aimbot", Items = {
...
function Hermes.Nospread( ucmd, angle, positive )
	...
	local func = Hermes.sys.CompensateWeaponSpread
	if ( usingDeco ) then 
		func = Hermes.hake.SpeedSpread
	...
	elseif ( Hermes.features['nospread'] != 1 ) then
		if ( usingDeco ) then
			return Hermes.hake.SpeedSpread( currentseed || 0, ( angle || ply:GetAimVector():Angle() ):Forward(), vecCone ):Angle()
		else
			return Hermes.sys.CompensateWeaponSpread( ucmd, Vector( -cone, -cone, -cone ) || 0, angle:Forward() || ply:GetAimVector():Angle() ):Angle()
...
function Asus:GetAll( bool )
	if ( !bool && !Asus.loaded ) then return end
	
	local mats = {}
	for k, v in pairs( Asus.files ) do
		local folder = file.Find( "../materials/" .. v .. "/*.vmt" )
		for u, e in pairs( folder ) do
			mats[ e ] = v
		end
	end
	...
	for k, v in pairs( mats ) do
		local name = string.gsub( k, ".vmt", "" )
		local mat = Material( v .. "/" .. name )
		
		mat:SetMaterialFloat( "$alpha", bool == true && 0.9 || 1 )
		matnum = matnum + 1
	end
	print( "Material number: " .. matnum )
	
	Asus.loaded = true
end

local looped = false
function Hermes:AsusWalls()
...
Hermes.bindfunctions = { "Menu", "Presets", "Aiming", "Trigger", "SpeedHack", "PropKill" }
	...
	button.DoClick = function()
		if ( combobox:GetSelectedItems() && combobox:GetSelectedItems()[1] ) then
			...
			for k, v in pairs( filecontents ) do
				...
				if ( str && str[ 2 ] ) then
					Hermes.cmd.LocalCommand( str[ 1 ] .. " " .. tonumber( str[ 2 ] ) )
				end
			end
		end
	end
