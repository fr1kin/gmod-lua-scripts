...
function Hermes:DetourFunction( typ )
...
Hermes:DetourFunction( "Angle" )
Hermes:DetourFunction( "CUserCmd" )
Hermes:DetourFunction( "Entity" )
Hermes:DetourFunction( "Player" )
Hermes:DetourFunction( "Vector" )
...
Hermes.detours		= {}
Hermes.override		= {}
Hermes.friends		= ( file.Exists( "hermes_lists_friends.txt" ) && KeyValuesToTable( file.Read( "hermes_lists_friends.txt" ) ) || {} ) -- Flapadar's method
...
Hermes.convars = {
	{ Item = "Aimbot", Setting = {
			{ Tab = "Aimbot", Items = {
					...
					{ ConVar = "prediction", Value = 1, Contents = { "Normal", "Framerate", "Velocity", "Division" }, Desc = "Prediction:", Type = "multichoice", None = true },
			...
			{ Tab = "Accuracy", Items = {
					...
					{ ConVar = "nospreadbase", Value = 0, Desc = "No-spread Base", Type = "checkbox" },
				}
			},
		},
...
function Hermes:GetAllPresetFiles()
	local preset = file.Find( "../data/*.txt" )
...
//------------------------------
// By noPE
// I SHOULD GET THEM TO WORK PROPERLY
...
function hook.Call( name, gm, ... )
	...
	return Hermes.copy.hook.Call( name, gm, ... )
end
...
function Hermes.ModuleCheck()
	if ( tobool( #file.Find( "../lua/includes/modules/gmcl_hake.dll" ) ) ) then
		if ( !Nospread ) then
			require( "hake" )
			package.loaded.hake = nil
		
			Hermes.loaded.hake = true
			Hermes.hake = {
				SpeedSpread = Nospread,
				FindPrediction = GetPrediction,
			}
	
			_G.Nospread = nil
			_G.GetPrediction = nil
		
			print( "Included 'gmcl_hake'" )
		end
	end
...
function _R.Player.ConCommand( ... )
	local args = { ... }
	local calldirectory = Hermes.copy.debug.getinfo(2)['short_src']
	if ( calldirectory && calldirectory == Hermes.path ) then return Hermes.copy.ConCommand( ... ) end
	if ( Hermes.getcvars[ string.lower( args ) ] ) then
		Hermes:AddLog( "Player.ConCommand(" .. string.format( "%s", tostring( args ) ) .. ")" )
		return 
	end
	return Hermes.copy.ConCommand( ... )
end
*/
	...
	for t, u in pairs( v.Setting ) do
		for m, n in pairs( u.Items ) do
			...
			elseif( n.Type == "multichoice" ) then
				--[[if ( !string.find( cvar, "font" ) && !string.find( cvar, "prediction" ) ) then
...
local function GetGamemode( ... )
	local args, gt1, gt2 = { ... }, GAMEMODE.Name, GetConVarString( "sv_gamemode" )
	if ( type( args ) != "table" ) then return end
...
function Cones.CustomType( cone, w )
	if ( !Hermes.features['nospreadbase'] ) then return nil end
	if ( !cone ) then return end
	local ply = LocalPlayer()
	if ( GetGamemode( "garry theft auto", "gta" ) ) then
		if ( ( type( w.Base ) == "string" ) && ( w.Base == "civilian_base" ) ) then
			local v = cone
			if  ( ply:KeyDown( IN_DUCK ) ) then
				v = math.Clamp( cone / 1.5, 0, 10 )
			...
			v = v + ( w:GetNWFloat( "Recoil", 0 ) / 3 )
			return Vector( v, 0, 0 )
		end
		return Vector( cone, cone, cone )
	elseif( GetGamemode( "rpdm", "wca" ) ) then
		if ( w.Base == "weapon_cs_base2" ) then
			return cone
		end
	elseif( GetGamemode( "darkrp", "zombrp" ) ) then
	...
	elseif( GetGamemode( "fortwarsx" ) ) then
...
local function GetCone( w )
	...
	local gamemodeCone = Cones.CustomType( c, w )
	if ( gamemodeCone ) then
		return gamemodeCone
	end
	
	/*if ( ( w.Base == "weapon_mad_base_sniper" ) && w:GetDTBool( 3 ) ) then 
		c = c * w.data.Cone 
		return c 
	end*/
...
_R.Entity.FireBullets = function( ent, bullet )
	...
	Hermes.VectorSpread[ w:GetClass() ] = bullet.Spread
	...
	if ( w && w:IsValid() ) then
		...
		if ( !Cones.Weapons[ class ] ) then
			if ( Hermes.VectorSpread[ class ] ) then
				...
				local conevec = Vector( 0, 0, 0 ) - Hermes.VectorSpread[ class ] || Vector( 0, 0, 0 )
...
local function PredictWall( tstart, tend, tdir, hitdis, ignore, tr )
	-- math skillz omg
	tdir['x'] = tend['x'] - tstart['x']
	tdir['y'] = tend['y'] - tstart['y']
	tdir['z'] = tend['z'] - tstart['z']
	
	local length = VectorLength( vec )
	tdir['x'] = tdir['x'] / length
	tdir['y'] = tdir['y'] / length
	tdir['z'] = tdir['z'] / length
	
	tend['x'] = tdir['x'] * hitdis + tstart['x']
	tend['y'] = tdir['y'] * hitdis + tstart['y']
	tend['z'] = tdir['z'] * hitdis + tstart['z']
	
	local length1, length2
	
	if( tstart['x'] == tend['x'] && tstart['y'] == tend['y'] && tstart['z'] == tend['z'] ) then
		tr.endpos = Vector( tend['x'], tend['y'], tend['z'] )
		return
	end
...
local function MadCow( bouncenum, attacker, tr, dmginfo, isplayer )
	local max - 16
	if w.Primary.Ammo == "AirboatGun" then
		max = 18
	elseif w.Primary.Ammo == "Gravity" then
		max = 8
	elseif w.Primary.Ammo == "AlyxGun" then
		max = 12
	elseif w.Primary.Ammo == "Battery" then
		max = 14
	elseif w.Primary.Ammo == "StriderMinigun" then
		max = 20
	elseif w.Primary.Ammo == "SniperPenetratedRound" then
		max = 16
	elseif w.Primary.Ammo == "CombineCannon" then
		max = 20
	end
...
function Hermes:Autowall()
	local ply, dmg = LocalPlayer(), nil
	
	local w = ply:GetActiveWeapon()
	if ( w && w:IsValid() ) then
		if ( w.Base == "weapon_mad_base" ) then
			if ( Hermes.Bullet[ w:GetClass() ] ) then
				local tbl = Hermes.Bullet[ w:GetClass() ]
				dmg = tbl.Damage || nil
			end
		end
	end
...
function Hermes:TargetVisible( e )
	//if ( Hermes:Autowall() ) then return true end
...
*/----------------------------------------
Hermes.cmds.ColorMsg( "About to make functions...\n", Color( 0, 0, 255 ) )
local Asus = {}
/*
...
function Asus:GetAll( bool )
	...
	Hermes.cmds.SetWorldTransparent( bool == true && 1 || 0 )
...
function Hermes:FilterColorsChams( e )
	...
	if ( IsRagdoll( e ) || IsVehicle( e ) ) then isVis = Hermes:GetColor( "entityespvis" ); Hermes:GetColor( "entityespnovis" ) end
	return isVis, notVis
end
...
function Hermes.SeconedChams()
	...
	for k, e in ipairs( Hermes.worldents.all ) do
		if ( Hermes:ValidTarget( e, "chams" ) ) then
			if ( tonumber( Hermes.features['walltype'] ) != 4 && tonumber( Hermes.features['walltype'] ) != 5 && Hermes.features['visiblechams'] && tonumber( Hermes.features['walltype'] ) != 1 ) then
				local isVis, notVis = Hermes:FilterColorsChams( e )
				e:SetMaterial( mat )
				e:SetColor( isVis )
			else
...
function Hermes.NameSteal()
	...
	if ( Hermes.features['namesteal'] ) then
		if ( !e:IsAdmin() && !( e:GetFriendStatus() == "friend" ) && e != ply ) then
			Hermes.cmds.RunCommand( "name " .. e:Nick() .. string.char( 03 ) ) //" "
		end
	end
end
...
function Hermes:PresetMenu()
	...
	button.DoClick = function()
		if ( combobox:GetSelectedItems() && combobox:GetSelectedItems()[1] ) then
			...
			local name = "hermes_preset_" .. filename .. ".txt"
			
			local filecontents = string.Explode( "\n", file.Exists( name ) && file.Read( name ) || "missing" )
			for k, v in pairs( filecontents ) do
				local str = string.Explode( "=", v )
				if ( str && str[ 2 ] ) then
					Hermes.cmds.RunCommand( str[ 1 ] .. " " .. tonumber( str[ 2 ] ) )
				end
			end
		end
	end
	...
	button1.DoClick = function()
		...
		local myVars = ""
		for k, v in pairs( Hermes.getcvars ) do
			local getVal = GetConVarNumber( v )
			myVars = myVars .. v .. "=" .. getVal .. "\n"
		end
		...
		local fullname = "hermes_preset_" .. filename .. ".txt"
		file.Write( fullname, myVars )
