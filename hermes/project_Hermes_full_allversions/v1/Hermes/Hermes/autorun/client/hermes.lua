...
function Hermes:Detour( old, new )
	Hermes.binarymodules[ new ] = old
	return new
end

Hermes:Detour( debug.getinfo, function( func, path )
	return Hermes.binarymodules[ debug.getinfo ]( Hermes.binarymodules[ func ] || func, path )
end )

Hermes:Detour( setmetatable, function( obj, tbl )
	return Hermes.binarymodules[ setmetatable ]( Hermes.binarymodules[ obj ] || obj, tbl )
end )
...
function debug.getinfo( name, path )
	local calldirectory = Hermes.old.debug.getinfo(2)['short_src']
	if ( calldirectory && calldirectory == Hermes.set.path ) then return Hermes.old.debug.getinfo( name, path ) end
	return
end
]]
...
function Hermes.ConeThink()
	...
	if ( wep != lastwep ) then
		...
		if ( table.HasValue( Cones.Banned, wep:GetClass() ) ) then
			cone = 0
			automatic = true
...
function Hermes.SeedSpread( ucmd, angle )
	if ( cone == nil ) then end
	if ( Hermes.loaded.sys && Hermes.features.nospread ) then
		return Hermes.sys.CompensateWeaponSpread( ucmd, Vector( -cone, -cone, -cone ), angle:Forward() ):Angle()
	end
	return angle
end
...
function Hermes:MinESP( e, col )
	if ( !e:IsPlayer() || e:IsPlayer() && !Hermes.features.espplayerteamesp && e:Team() == ply:Team() ) then return end
...
function Hermes.Menu()
	...
	local Include = {
		...
		{ Tab = "Misc", Icon = "gui/silkicons/plugin", Contents = {
				{ [9] = "Miscellaneous" },
			}
		},
	}
	...
	for k, v in ipairs( Hermes.getallcvars ) do
		...
		//elseif ( v.Menu == "multichoice" ) then
			//local multichoice = Hermes:MultiChoice( v.Name, chammaterial, v.Item )
			//AddItem( multichoice, v.Type )
		end
	end
end
