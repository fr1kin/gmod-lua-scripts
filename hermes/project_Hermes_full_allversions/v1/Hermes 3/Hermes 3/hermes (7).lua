...
Hermes:CreateClientConVar( "aim_norecoil", true, "norecoil", "Norecoil", "aim" )
...
function Hermes:TargetLocation( e )
	...
	if ( m == "models/error.mdl" ) then								pos = Hermes:GetPos( e, e:OBBCenter() + Vector( 0, 0, 20 ) ) end
...
function Hermes.Aim( ucmd )
	...
	if ( Hermes:GetValue( Hermes.features.norecoil, 1 ) ) then
	...
	//aim = aim + ( tar:GetVelocity() * ( 7 / tar:GetVelocity():Length() ) )
...
function Hermes.NoRecoil( ply, origin, angles, FOV )
	if ( Hermes:GetValue( Hermes.features.norecoil, 0 ) ) then return end
...
function Hermes.EnabledSpeed()
	if ( Hermes:GetValue( Hermes.features.speedhack, 1 ) ) then
		RunConsoleCommand( "hermes_sv_cheats", 1 )
...
function Hermes.DisableSpeed()
	RunConsoleCommand( "hermes_host_timescale", 1 )
end

// Enable HLDJ
function Hermes.AllowHLDJ()
	RunConsoleCommand( "hermes_sv_allow_voice_from_file", 1 )
	RunConsoleCommand( "hermes_voice_inputfromfile", 1 )
end
...
Hermes:AddCommand( "hermes_enablehldj", function() Hermes.AllowHLDJ() end )
