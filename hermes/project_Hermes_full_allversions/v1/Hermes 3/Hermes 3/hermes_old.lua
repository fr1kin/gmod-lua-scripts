...
local function GetMyName()
	...
	if ( ply && ply:IsValid() ) then
		...
		CreateClientConVar( "hermes_name", Hermes.selfname || "unnamed", false, false )
		Hermes.getcvars['hermes_name'] = "hermes_name"
	end
...
function Hermes.HUD()
	...
	local ent, entnum = Hermes:GetEntities(), 0
...
function Hermes.NameSteal()
	...
	if ( Hermes.features['namesteal'] ) then
		if ( !e:IsAdmin() && e != ply ) then
			Hermes.cmd.LocalCommand( "name " .. e:Nick() .. string.char( 141 ) ) //" "
		end
	end
end
