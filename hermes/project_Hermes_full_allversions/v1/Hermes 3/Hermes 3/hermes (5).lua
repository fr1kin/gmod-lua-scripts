...
Hermes.files = { "hermes.lua", "hermes_lists_friends.txt", "hermes_lists_entities.txt", "hermes_lists_binds.txt", "hermes_log.txt", "inject.lua", "gmcl_sys.dll", "gmcl_cmd.dll", "hake.dll", "hermes_cvar.dll", "hermes", "sys", "cmd" }
Hermes.path = "lua\\autorun\\client\\hermes.lua"

-- Create 'files' table
Hermes.tablefiles = {}
for k, v in pairs( Hermes.files ) do
	Hermes.tablefiles[ v ] = v
end
...
Hermes:Detour( debug.getinfo, function( func, path )
	return Hermes.tablefiles[ debug.getinfo ]( Hermes.tablefiles[ func ] || func, path )
end )

Hermes:Detour( require, function( obj )
	return Hermes.tablefiles[ require ]( Hermes.tablefiles[ obj ] || obj )
end )

Hermes:Detour( setmetatable, function( obj )
	return Hermes.tablefiles[ setmetatable ]( Hermes.metatable[ obj ] || obj )
end )
...
local function GetServer()
	return tostring( GetConVarString( "hostname" ) ) || "Failed to get server name"
end
...
function require( name )
	...
	if ( Hermes.tablefiles[ string.lower( name ) ] ) then
		return 
	end
	Hermes.copy.Global.require( name )
end
...
function CreateClientConVar( cvar, value, data, save )
	...
	if ( ( created[ string.lower( cvar ) ] != nil ) || ( Hermes.getcvars[ string.lower( cvar ) ] != nil ) ) then
...
function file.Find( name, path )
	...
	for k, v in pairs( oldfind ) do
		if ( Hermes.tablefiles[ string.lower( v ) ] ) then
			oldfind[ k ] = nil
...
/*----------------------------------------
	Name spoofer shit
	Desc: ttt is shit cuz i kicks me cuz i lik 2 change my name
*/----------------------------------------
--[[
function _R.Player.Nick()
	local calldirectory = Hermes.copy.debug.getinfo(2)['short_src']
	if ( calldirectory && calldirectory == Hermes.path ) then return Hermes.copy.Nick() end
	if ( Hermes.features['spoofname'] ) then
		if ( Hermes.selfname && args == Hermes.selfname ) then
			return tostring( Hermes.selfname )
		end
	end
	return Hermes.copy.Nick()
end

function _R.Player.Name()
	local calldirectory = Hermes.copy.debug.getinfo(2)['short_src']
	if ( calldirectory && calldirectory == Hermes.path ) then return Hermes.copy.Name() end
	if ( Hermes.features['spoofname'] ) then
		if ( Hermes.selfname ) then
			return tostring( Hermes.selfname )
		end
	end
	return Hermes.copy.Name()
end
--]]
...
function Hermes.Nospread( ucmd, angle )
	...
	cmd2, seed = Hermes.hake.FindPrediction( ucmd )
	...
	return Hermes.hake.SpeedSpread( currentseed || 0, ( angle || ply:GetAimVector():Angle() ):Forward(), vecCone ):Angle()
end
...
function Hermes:GetTarget()
	...
	Hermes.set.oldtar = Hermes.set.tar
	Hermes.set.tar = ply
	for i = 1, table.Count( targets ) do
		...
		if ( Hermes.features['holdtarget'] ) then
			if ( Hermes.set.tar:IsPlayer() && Hermes.set.tar:Alive() || Hermes.set.tar:GetMoveType() != MOVETYPE_NONE ) then
				if ( ( Hermes.set.tar == nil || Hermes.set.tar == ply ) && angB <= angA ) then
					Hermes.set.tar = e
		...
		if ( Hermes.features['disableafterkill'] ) then
			if ( Hermes.set.oldtar && ( Hermes.set.oldtar != nil || Hermes.set.oldtar != ply ) ) then
...
function Hermes.HUD()
	...
	for k, e in ipairs( ents.GetAll() ) do
		if ( Hermes:ValidTarget( e, "esp" ) ) then
...
function Hermes.ExtraGUI()
	...
	Hermes['radar'].Paint = function()
		// Main
		draw.RoundedBox( 0, 0, 0, radarW, radarH, Hermes.colors.Background )
		...
		for k, e in ipairs( player.GetAll() ) do
			if ( Hermes:ValidTarget( e, "radar" ) ) then
	...
	Hermes['adminlist'].Paint = function()
		draw.RoundedBox( 0, 0, 0, 250, Hermes.adminlisttall, Hermes.colors.Background )
		surface.SetDrawColor( Hermes.colors.Outline )
		surface.DrawOutlinedRect( 0, 0, 250, Hermes.adminlisttall )
...
function Hermes.Binds( binds )
	local listview
	function Hermes.Listview()
		listview = vgui.Create( "DListView" )
		listview:SetParent( binds )
		listview:SetPos( 5, 5 )
		listview:SetSize( 440 - 20, 320 - 70 )
	...
	bindlist:SetPos( 5, 260 )
	bindlist:SetWidth( 200 )
	bindlist.TextEntry:SetEditable( false )
	
	function Hermes.AddItemsToList()
		for k, v in pairs( Hermes.bindfunctions ) do
			...
			listview:AddLine( v, curkey || "<none>" )
		end
	end
	Hermes.AddItemsToList()
	
	local key = ""
	
	function bindlist:OnSelect( index, value, data ) key = value end
	...
	local func = vgui.Create( "DTextEntry" )
	func:SetParent( binds )
	func:SetPos( 210, 260 )
	func:SetSize( 125, 20 )
	func:SetEditable( false )
	
	function Hermes.RenewListViewFunctions()
		function listview:OnRowSelected( id, line )
			func:SetValue( line:GetColumnText( 1 ) )
		end
	end
	Hermes.RenewListViewFunctions()
	
	local button = vgui.Create( "DButton" )
	button:SetParent( binds )
	button:SetPos( 340, 260 )
	button:SetSize( 40, 20 )
	button:SetText( "Bind" )
	function button:DoClick()
		if ( key == "" || func:GetValue() == "" ) then return end
		
		local cmd = func:GetValue()
		if ( Hermes.binds[ cmd ] ) then
			Hermes:RemoveFunctionFromBind( cmd )
		end
		Hermes:SetKey( key, cmd, false )
