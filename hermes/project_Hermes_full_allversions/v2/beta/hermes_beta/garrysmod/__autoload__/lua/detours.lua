...
local function Unlock()
	if( _G['rawset'] ) then
		...
		_G['__metatable'] = false;
		hook['__metatable'] = false;
		concommand['__metatable'] = false;
		
		_G['__eq'] = true;
		hook['__eq'] = true;
		concommand['__eq'] = true;
	end
end
Unlock()
...
local function ProtectTable()
	if( !rawget || !rawset ) then return end
	_G['HERMES'] = nil
...
local function CallHook( name, gm, ... )
	...
	if( name == "PlayerBindPress" && IsValidCommand( ... ) ) then return false end
