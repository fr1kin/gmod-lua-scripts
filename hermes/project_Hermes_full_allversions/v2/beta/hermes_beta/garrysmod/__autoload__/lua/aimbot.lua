...
HERMES:AddSetting( "aim_frame", false, "FramePrediction", "General" )
...
HERMES:AddSetting( "aim_smooth", 0, "Smoothaim", "General", 0.1, 50, true )
...
/* --------------------
	:: Prediction
*/ --------------------
local oldOrg, oldestOrg = Vector( 0, 0, 0 ), Vector( 0, 0, 0 )
function HERMES:Prediction( tar, compensate )
	local ply = LocalPlayer()
	//compensate = compensate + ( tar:GetVelocity() / 45 - ply:GetVelocity() / 45 )
	if( HERMES.item['frameprediction'] ) then
		...
		return HERMES.hermes.LagCompensation( compensate )
	end
end
...
function HERMES:FilterAimTargets()
	...
	local targets, ply = HERMES.item['npcs'] && ents.GetAll() || player.GetAll(), LocalPlayer()
...
function HERMES.CalcView( ply, origin, angles, FOV )
	...
	if( HERMES.item['novisrecoil'] && ( HERMES['_lock'] == false ) ) then
