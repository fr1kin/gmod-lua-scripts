...
HERMES:AddSetting( "aim_hitbox", true, "Hitbox", "General" )
HERMES:AddSetting( "aim_prediction", false, "Prediction", "General" )
HERMES:AddSetting( "aim_los", true, "LOSChecks", "General" )
HERMES:AddSetting( "aim_obb", false, "OBBAiming", "General" )
...
HERMES:AddSetting( "aim_fakeangle", false, "FakeAngle", "General" )
HERMES:AddSetting( "aim_awall", false, "Autowall", "General" )
...
HERMES:AddSetting( "aim_offset", 0, "Offset", "General", -100, 100, false )
...
function HERMES:FakeAngles( ucmd )
	...
	if( ucmd:KeyDown( IN_ATTACK | IN_ATTACK2 | IN_USE ) ) then return end
	if( LocalPlayer():GetMoveType() == MOVETYPE_LADDER ) then return end
...
function HERMES:GetPos( e, noedit )
	...
	if( noedit ) then return pos end
	if( HERMES.item['obbaiming'] ) then pos = e:LocalToWorld( e:OBBCenter() ) end
...
function HERMES:IsVisible( e )
	if( !HERMES.item['loschecks'] ) then return true end
	local ply = LocalPlayer()
...
/* --------------------
	:: Aimbot
*/ --------------------
local SetAngles = HERMES.hermes.SetViewAngles //_R["CUserCmd"].SetViewAngles

local function SetViewAngles( ucmd, ang )
	local x, y, z = ang.p, ang.y, ang.r
	HERMES.hermes.SetViewAngles( ucmd, Vector( x, y, z ) )
end
...
function HERMES:Aimbot( ucmd )
	...
	compensate = HERMES:Prediction( tar, compensate ) + Vector( 0, 0, HERMES.item['offset'] )
	
	local hitbox = Angle( 0, 0, 0 )
	if( HERMES.item['hitbox'] && tar:IsPlayer() ) then
		//ang = HERMES:GetPos( tar )
		//if( HERMES.item['prediction'] ) then
			//ang = HERMES.hermes.LagCompensation( ang, ply:Ping() )
		//end
		local target_hitbox = HERMES.hermes.GetHitbox( 12, tar:EntIndex() )
		//local self_hitbox = HERMES.hermes.GetHitbox( 12, ply:EntIndex() )
		
		//print( "\n" .. target_hitbox )
		//hitbox = ( target_hitbox - self_hitbox ):Angle()
		//print( hitbox, type( hitbox ) )
	end
...
local PK = {
	...
	throw = false,
	fakeang = Angle( 0, 0, 0 ),
	throwang = Angle( 0, 0, 0 ),
}

function HERMES:PropKill( cmd )
	local ply = LocalPlayer()
	if( HERMES.GetGamemode( "terror town", "terrortown" ) ) then
		if( PK.fake && !PK.throw ) then
			PK.throwang = cmd:GetViewAngles()
			PK.fakeang = PK.throwang - Angle( 0, 180, 0 )
			local move = Vector( cmd:GetForwardMove(), cmd:GetSideMove(), 0 )
			local norm = move:GetNormal()
			local ang = ply:GetAimVector():Angle()
			local set = ( norm:Angle() + ( PK.fakeang - ang ) ):Forward() * move:Length()
			cmd:SetForwardMove( set.x )
			cmd:SetSideMove( set.y )
		elseif( PK.fake && PK.throw ) then
			PK.throw = false
			SetViewAngles( cmd, PK.fakeang )
		else
			PK.throwang = cmd:GetViewAngles()
			PK.fakeang = PK.throwang
		end
	end
end

HERMES:AddCommand( "+hermes_pk", function() PK.fake = true end )
HERMES:AddCommand( "-hermes_pk", function() PK.fake = false; PK.throw = true end )
...
function HERMES.CalcView( ply, origin, angles, FOV )
	...
	if( PK.fake && HERMES.GetGamemode( "terror town", "terrortown" ) ) then
		ang = PK.fakeang
	end
	
	if( HERMES.item['novisrecoil'] ) then
		local view = GAMEMODE:CalcView( ply, origin, ang, FOV ) || {}
			view.angles = ang
			view.angles.r = 0
		return view
	end
