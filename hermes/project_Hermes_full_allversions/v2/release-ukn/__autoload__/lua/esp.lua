...
HERMES:AddSetting( "esp_barrel", true, "Barrel", "Player" )
...
function HERMES:Barrel( e, col )
	if( !HERMES.item['barrel'] ) then return end
	if( !e:IsPlayer() || e:IsPlayer() && HERMES.IsDormant( e ) ) then return end
	local mat = HERMES:CreateLaserObject( string.format( "%c%c%c", col.r, col.g, col.b ), col )
	local pos, hitPos = HERMES:GetPos( e, true ), e:GetEyeTrace().HitPos
	cam.Start3D( EyePos(), EyeAngles() )
		render.SetMaterial( mat )
		render.DrawBeam( pos, hitPos, 5, 0, 0, Color( col.r, col.g, col.b, 255 ) )
	cam.End3D()
end
...
function HERMES:Optical( e, col, maxX, minX, maxY, minY )
...
function HERMES:FilterTargets( e, mode )
	...
	elseif( mode:lower() == "aim" ) then
		...
		if ( tonumber( HERMES.item['fov'] ) != 180 ) then
			...
			local ang = ( HERMES:GetPos( e ) - ply:GetShootPos() ):Angle()
...
/* --------------------
	:: HealthBar
*/ --------------------
function HERMES:HealthBar( e, col )
	...
	if ( e:Health() <= 50 ) then
		for i = 1, 4 do
			...
			surface.SetDrawColor( col )
			surface.DrawLine( minX - pos, ( hpmaxY + 1 ), minX - pos, maxY )
			pos = pos + 1
		end
	else
		for i = 1, 4 do
...
function HERMES:HUD()
	for k, e in ipairs( ents.GetAll() ) do
		...
		if( valid == true ) then
			...
			// Barrel
			HERMES:Barrel( e, color )
			
			// HPBar
			HERMES:HealthBar( e, color )
			
			// Box ESP
			if( HERMES.item['optical'] && !HERMES.IsDormant( e ) || HERMES.item['far'] && HERMES.IsDormant( e ) ) then
				HERMES:Optical( e, color, maxX, minX, maxY, minY )
			end
...
/* --------------------
	:: OnEntityCreated
*/ --------------------
function HERMES.OnEntityCreated( e )
	if( !ValidEntity( e ) ) then return end
	if( e:IsPlayer() ) then
		spawnhp[e] = e.Health && e:Health() || 100
	end
end
