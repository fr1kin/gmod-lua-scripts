...
function HERMES:HealthBar( e, col, mpos )
	if( !HERMES.item['dynamicbar'] ) then return end
	if( !e:IsPlayer() ) then return end
	
	local ply = LocalPlayer()
	
	local maxhealth, health = spawnhp[e] || 100, e:Health()
	local mx, mw = maxhealth / 4, health / 4
	
	local dis, var = math.Round( e:GetPos():Distance( ply:GetShootPos() ) ); var = dis / 30
	local pos = e:GetPos():ToScreen()
	
	pos.x = pos.x - ( mx / 2 )
	pos.y = pos.y + 10
	
	//FillRGBA( pos.x - 1, pos.y - 1, mx + 2, 4 + 2, BLACK )
	//FillRGBA( pos.x, pos.y, mw, 4, col )
end
