...
//HERMES.files["ini.lua"] = true
HERMES.files["!!!install.lua"] = true
HERMES.files["gmod_cvars_cl"] = true
...
function HERMES:AddSetting( name, value, desc, menu, ... )
	...
	if( !HERMES.convar[name] ) then
		...
		local cvarinfo = { Name = cvar, ConVar = convar:GetName(), Value = val, Desc = desc, Menu = menu, Max = max, Min = min, Decimal = deci, Type = mtype }
		HERMES.menu[desc:lower()] = cvarinfo
		HERMES.menu[#HERMES.menu + 1] = cvarinfo
...
function HERMES:RunCommand( var, val )
	val = tostring( val )
	local cvar = string.format( "%s %s", var, val )
...
local function GetGamemode( ... )
	...
	for k, v in pairs( args ) do
		if ( ( type( v ) == "string" ) && ( string.find( gt1:lower(), v ) || string.find( gt2:lower(), v ) ) ) then
			n = n + 1
		end
	end
