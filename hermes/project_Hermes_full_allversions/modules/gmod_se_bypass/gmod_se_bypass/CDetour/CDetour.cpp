...
void* CDetour::memset_s( void * ptr, int value, size_t num )
{
	MEMORY_BASIC_INFORMATION mbi;
	VirtualQuery( ( void* )ptr, &mbi, sizeof( mbi ) );
	VirtualProtect( mbi.BaseAddress, mbi.RegionSize, PAGE_EXECUTE_READWRITE, &mbi.Protect );
	void* pvRetn = memset( ptr, value, num );
	VirtualProtect( mbi.BaseAddress, mbi.RegionSize, mbi.Protect, &mbi.Protect );
	FlushInstructionCache( GetCurrentProcess( ), ( void* )ptr, num );
	return pvRetn;
}

void *CDetour::Create(BYTE *orig, const BYTE *det, int iPatchType, int len)
{
	if( iPatchType == DETOUR_TYPE_CLASS )
	{
		return this->DetourClassFunc( orig, det, len );
	}
